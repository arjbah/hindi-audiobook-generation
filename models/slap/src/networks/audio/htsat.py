...
try:
    ...
    print("Install torchlibrosa first.")
...
# The Core of HTSAT
class HTSATSwinTransformer(nn.Module):
    ...
    def __init__(self, spec_size=256, patch_size=4, patch_stride=(4, 4),
                 ...
                 mel_bins=64, sample_rate=44100, spec_window_size=1024, hop_size=480, fmin=50, fmax=14000,
                 ...
                 enable_fusion=False, fusion_type='None',
                 spec_augment=False, **kwargs
                 ):
        super(HTSATSwinTransformer, self).__init__()
        ...
        if precomputed_lms:
            ...
            self.spectrogram_extractor = Spectrogram(n_fft=spec_window_size, hop_length=hop_size,
                                                     win_length=spec_window_size, window=window, center=center,
                                                     pad_mode=pad_mode,
                                                     freeze_parameters=True)
            self.spectrogram_extractor.double()
            # Logmel feature extractor
            self.logmel_extractor = LogmelFilterBank(sr=sample_rate, n_fft=spec_window_size,
        ...
        self.spec_augmenter = SpecAugmentation(time_drop_width=64, time_stripes_num=2,
                                                freq_drop_width=8, freq_stripes_num=2) if spec_augment else nn.Identity()
        ...
        if False:  # related to attention maps, not supported for now
        ...
        # this motherfucker is not used anywhere and messes up DDP
        ...
        if checkpoint_path:
            # checkpoint keys all start with `sed_model.<key_name>`
            checkpoint = torch.load(checkpoint_path, map_location=torch.device("cpu"))["state_dict"]
            try:
                self.load_state_dict({k[10:]: v for k, v in checkpoint.items()})
            except RuntimeError as e:
                log.warning(e)
                self.load_state_dict({k[10:]: v for k, v in checkpoint.items()}, strict=False)
            # self.bn0 = nn.BatchNorm2d(mel_bins)
        else:
    ...
    def forward_features(self, x, longer_idx=None):
        ...
        # display the attention map, if needed: it messes up DDP so fuck it
        if True:
            output_dict = {
    ...
    def reshape_wav2img(self, x):
        ...
        # print(x.shape, target_T, target_F)
        # raise IndexError

        assert F <= target_F, "the wav size should less than or equal to the swin input size"
        # to avoid bicubic zero error
        if T < target_T:
            x = torch.nn.functional.pad(x, (0, 0, 0, target_T - T))
    ...
    def forward(self, x: torch.Tensor, mixup_lambda=None, infer_mode=False,
        ...
        if x.dim() == 2 or (x.dim() == 3 and (x.shape[1] in [1,2])): # waveform
            if x.dim() == 3: x = x.mean(dim=1)
            
            #switch to full precision to avoid nans
            dtype = x.dtype
            x = x.double()
        ...
        else:  # spectrogram in timesteps_last
            x = x.transpose(2, 3)
