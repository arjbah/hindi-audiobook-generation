#!/usr/bin/env python3
"""
Sentiment-Guided IndicParlerTTS Narrator Pipeline — Hindi
==========================================================
...
New vs. indic_narrator_pipeline.py
-----------------------------------
  ...
  · SentimentClassifier   — tabularisai/multilingual-sentiment-analysis BERT
                            (multilingual — handles Devanagari natively)
  · EmotionAssigner       — score + Hindi text heuristics → IndicParler emotion
...
------------
  ...
  Per-chunk loop
    ...
    → EmotionAssigner: score + Hindi keyword heuristics → (emotion, expressivity_phrase)
...
Usage
-----
  python sentiment_narrator_hin.py
  python sentiment_narrator_hin.py --story transcripts/long_story_transcript.txt \\
      --output output/HIN_sentiment/
"""
...
class PipelineConfig:
    ...
    # ── Chunking ──────────────────────────────────────────────────────────────
    max_chunk_chars: int = 280       # Devanagari chars; 280 ≈ 300 English equivalents
...
# ─────────────────────────────────────────────────────────────────────────────
# TEXT CHUNKER  (Hindi — Devanagari dandas + standard punctuation)
# ─────────────────────────────────────────────────────────────────────────────

class TextChunker:
    """
    Splits Hindi/Devanagari text at sentence and paragraph boundaries,
...
# ─────────────────────────────────────────────────────────────────────────────
# SENTIMENT CLASSIFIER  — BERT text classification (multilingual)
# ─────────────────────────────────────────────────────────────────────────────

class SentimentClassifier:
    """
    Wraps tabularisai/multilingual-sentiment-analysis.
    Handles Devanagari (Hindi) natively — no transliteration needed.
...
# ─────────────────────────────────────────────────────────────────────────────
# EMOTION ASSIGNER  — maps BERT output + Hindi heuristics → IndicParler style
# ─────────────────────────────────────────────────────────────────────────────

# ── Hindi keyword patterns (Devanagari) ──────────────────────────────────────

# Dialogue: direct speech markers, quote marks, and speech verbs
_HI_DIALOGUE = re.compile(
    '[\u201c\u201d\u2018\u2019\'"]'          # curly and straight quote characters
    r'|(?:बोल[ेीं]?|कहा|पूछा|बोली|बोले|कहने|उत्तर(?:दिया)?|'
    r'मुस्कुराते\s*हुए|हँसते\s*हुए|फुसफुसाया|चिल्लाया|'
    r'बताया|समझाया|जवाब\s*दिया)',
)

# Imperative verbs — common Hindi command forms
_HI_IMPERATIVE = re.compile(
    r'\b(?:जाओ|आओ|करो|बताओ|देखो|सुनो|रुको|चलो|लो|दो|'
    r'बैठो|उठो|खाओ|पियो|पढ़ो|लिखो|बोलो|सोचो|समझो|'
    r'भागो|हटो|मत\s+(?:जाओ|करो|रोओ|डरो)|याद\s+करो|'
    r'ध्यान\s+दो|मान\s+लो)\b',
)

# Anger / wrath vocabulary
_HI_ANGER = re.compile(
    r'(?:क्रोध|गुस्स[ेाा]?|चिल्ला(?:या|ई|ना|ते)?|'
    r'क्रोधित|आक्रोश|रोष|आवेश|भड़क(?:ा|ी|ना|ते)?|'
    r'नाराज़?|गुस्से\s*में|चीख(?:ा|ी|ना|ते)?|'
    r'दाँत\s*पीस|मुट्ठी\s*भींच)',
)

# Fear / terror vocabulary
_HI_FEAR = re.compile(
    r'(?:डर(?:ा|ी|ना|ते|गया|गई)?|भय(?:भीत|ग्रस्त)?|'
    r'घबरा(?:या|ई|ना|ते|हट)?|आतंक|दहशत|काँप(?:ा|ी|ना|ते)?|'
    r'थरथरा(?:या|ई|ना|ते)?|भीत|सहम(?:ा|ी|गया|गई|ना)?|'
    r'ख़ौफ़?|भयानक|डरावन[ाी])',
)

# Disgust vocabulary
_HI_DISGUST = re.compile(
    r'(?:घृणा|नफ़?रत|विरक्त|घिन|जुगुप्सा|'
    r'नफरत\s*से|घृणित|अरुचि|वितृष्णा)',
)

# Happy / joyful vocabulary
_HI_HAPPY = re.compile(
    r'(?:खुश(?:ी|हाली)?|प्रसन्न(?:ता)?|हँस(?:ा|ी|ना|ते)?|'
    r'हर्ष(?:ित)?|आनंद(?:ित)?|प्रफुल्लित|उत्साह(?:ित)?|'
    r'मुस्कुरा(?:या|ई|ना|ते)?|मुस्कान|ख़ुशी|जश्न|'
    r'आह्लादित|प्रेम(?:पूर्ण)?|स्नेह)',
)

# Surprise / shock vocabulary
_HI_SURPRISE = re.compile(
    r'(?:अचानक|चौंक(?:ा|ी|गया|गई|ना)?|आश्चर्य(?:जनक|चकित)?|'
    r'हैरान(?:ी)?|विस्मय(?:ित)?|अचरज|चकित|स्तब्ध|'
    r'एकाएक|अप्रत्याशित|अविश्वसनीय)',
)

# Sadness vocabulary
_HI_SAD = re.compile(
    r'(?:दुख(?:ी|द|ित)?|उदास(?:ी)?|रो(?:या|ई|ना|ते)?|'
    r'आँसू|बिलख(?:ना|ते)?|विलाप|शोक(?:ाकुल)?|'
    r'दर्द|पीड़ा(?:दायक)?|ग़म|मातम|व्यथित|'
    r'रुलाई|सिसकी|तड़प(?:ना|ते)?)',
)

# News / report style markers
_HI_NEWS = re.compile(
    r'(?:के\s*अनुसार|रिपोर्ट|घोषणा|आधिकारिक(?:तौर\s*पर)?|'
    r'बताया\s*गया|समाचार|प्रवक्ता|सूत्र(?:ों)?)',
)
...
# Devanagari proper noun heuristic: 2+ consecutive words starting with capital-equivalent
# (Hindi doesn't use capitals, but story text may mix Roman names)
# Also detect common place/person name suffixes in Hindi narrative context
_HI_PROPER_CONTEXT = re.compile(
    r'(?:[A-Z][a-z]+(?:\s+[A-Z][a-z]+)+)'           # Roman names in Hindi text
    r'|(?:जी\b|साहब\b|बाबू\b|भाई\b|देवी\b|जान\b)',   # honorifics → named reference
)


class EmotionAssigner:
    """
    Maps BERT sentiment score + Hindi text heuristics to an IndicParler emotion
    token and a natural-language expressivity phrase.

    Decision logic
    --------------
    1. Map numeric score [-1, 1] to a sentiment bin
    2. Within the bin, apply Hindi keyword/pattern overrides
    3. Cross-bin checks: dialogue → Conversation, imperative → Command
    ...
    def _pick_emotion(self, text: str, score: float) -> str:
        has_anger    = bool(_HI_ANGER.search(text))
        has_fear     = bool(_HI_FEAR.search(text))
        has_disgust  = bool(_HI_DISGUST.search(text))
        has_happy    = bool(_HI_HAPPY.search(text))
        has_surprise = bool(_HI_SURPRISE.search(text))
        has_sad      = bool(_HI_SAD.search(text))
        has_dialogue = bool(_HI_DIALOGUE.search(text))
        has_command  = bool(_HI_IMPERATIVE.search(text))
        has_news     = bool(_HI_NEWS.search(text))
        has_proper   = bool(_HI_PROPER_CONTEXT.search(text))

        # Command and Conversation are cross-bin — check first
        if has_command and score >= -0.2:
            return "Command"

        # Very Negative  (score < -0.65)
        if score < -0.65:
        ...
        if score <= 0.2:
            ...
            if has_proper and not has_dialogue:
                return "Proper Noun"
            # Sad can leak into neutral bin for understated grief
            if has_sad:
                return "Sad"
            return "Narration"

        # Positive  (0.2 < score ≤ 0.65)
        if score <= 0.65:
            if has_surprise:
                return "Surprise"
            if has_dialogue:
...
class SentimentNarratorPipeline:
    """
    End-to-end sentiment-guided TTS for a Hindi story.
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        description="Sentiment-guided Hindi IndicParlerTTS pipeline",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    p.add_argument("--story",   default="transcripts/long_story_transcript.txt",
                   help="Path to story text file (UTF-8, Hindi/Devanagari)")
    p.add_argument("--output",  default="output/HIN_sentiment/",
    ...
    p.add_argument("--anchor-text", default=None,
                   help="Hindi seed sentence for anchor clip")
