...
class INF5Model(PreTrainedModel):
    ...
    def forward(self, text: str, ref_audio_path: str, ref_text: str):
        ...
        # Load reference audio & text without aggressive 15s clipping
        # This allows the Master Personality Print context to be fully utilized.
        ref_audio, ref_text = preprocess_ref_audio_text(ref_audio_path, ref_text, clip_short=False)

        
        self.ema_model.to(self.device)
        self.vocoder.to(self.device)
        
        # Perform inference with more stable parameters
        # nfe_step=64: more steps reduce ODE solver artifacts
        # cfg_strength=1.5: lower strength prevents "explosions" (white noise bursts)
        ...
        # Ensure the raw float audio is within [-1, 1] before any further processing
        max_val = np.abs(audio).max()
        if max_val > 0.99:
            audio = audio / (max_val + 1e-6)
            print(f"DEBUG: Applied peak normalization (max was {max_val:.2f})")
        ...
        if self.config.remove_sil:
            # Tune silence removal to be less aggressive (shorter min_silence_len)
            non_silent_segs = silence.split_on_silence(
                audio_segment,
                min_silence_len=500, # reduced from 1000
                silence_thresh=-55,  # slightly more sensitive
                keep_silence=300,    # reduced from 500
                seek_step=10,
            )
            if non_silent_segs:
                audio_segment = sum(non_silent_segs, AudioSegment.silent(duration=0))

        # Normalize loudness to a consistent standard
        target_dBFS = -20.0
