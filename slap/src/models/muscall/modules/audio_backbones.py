...
class AttentionPool2d(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x + self.positional_embedding[:x.size(0), None, :].to(x.dtype)  # (HW+1)NC
...
class ModifiedResNet(nn.Module):
    ...
    def forward(self, x, spec_ = True):
        
        
        spec = self.spec(x) if spec_ else x
        x = self.amplitude_to_db(spec.unsqueeze(1)) if spec_ else self.amplitude_to_db(spec)
