...
class IndicVoicesCLAPDataset(Dataset):
    def __init__(self, dataset_name, split="train", target_sr=48000, max_audio_len=10.24, max_text_len=128, muril_model_name="google/muril-base-cased"):
        ...
        # Audio length in samples (Preserving 10.24s length for shape safety)
    ...
    def __getitem__(self, idx):
        ...
        # Resample using stateless functional for efficiency (at 48kHz for LAION-CLAP)
...
def get_dataloader(split="train", batch_size=32, num_workers=4):
    ...
    return DataLoader(
        ...
        pin_memory=True # Optimization enabled
    )

def get_full_dataset():
    # Only load Rasa train and test splits to preserve full-eval codebase compatibility
    ...
    dataset = ConcatDataset([rasa_train, rasa_test])
    return dataset
